...
void checkTimer( byte srcId) {
  if ( SERIAL_TIMER_DISABLE == timer[ srcId ]) {
    ...
    if ( digit_idx[ srcId ] > 0 || _match[ srcId ] > 0 ) { // reset data --- havn't seen data for a while
        resetSML( srcId );
...
void readSML(byte inByte, byte srcId){
  ...
  if ( pattern_len <= match) {
    if ( digit_idx[srcId] >= DIGIT_VALUE_SIZE ) {
        // digit_idx[srcId]=0 ; // just to be safe !!!
    } else if ( match < row_len[srcId] ) {
      if (0 == digit_idx[srcId] /*&& 0 == digit[srcId][digit_idx[srcId]]*/) {
    ...
    if (digit_idx[srcId] < DIGIT_VALUE_SIZE) digit[srcId][digit_idx[srcId]] = 0; // reset .. just in case ... 
    match++;
  } else {
    match = 0;
  } // match
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (digit_idx[0] > DIGIT_MAX_VALUE_COUNT) {
      Serial.print("H");Serial.print(digit_idx[0]);Serial.print(digit_idx[1]);
    }    
  } else checkTimer( 0) ;
  ...
  if (mySerial_1.available() > 0) {
    ...
    if (digit_idx[1] > DIGIT_MAX_VALUE_COUNT) {
      Serial.print("S");Serial.print(digit_idx[0]);Serial.print(digit_idx[1]);
    }
  } else checkTimer( 1) ;


  if ((   (0 == digit_idx[0] && digit_idx[1] > 0) || digit_idx[0] >= DIGIT_MAX_VALUE_COUNT) 
      && ((0 == digit_idx[1] && digit_idx[0] > 0) || digit_idx[1] >= DIGIT_MAX_VALUE_COUNT))  {
